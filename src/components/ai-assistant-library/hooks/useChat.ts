...
export function useChat(initialConfig: ChatConfig = {
  ...
  const aiModelsResult = scope.run(() => useAIModels())
  const { currentModelId, currentModel, selectModel: selectAIModel } = aiModelsResult || { 
    currentModelId: ref(''), 
    currentModel: computed(() => undefined), 
    selectModel: () => {} 
  }
  ...
  function addMessage(message: Partial<Message>): Message {
    const newMessage: Message = {
      ...
      modelInfo: message.modelInfo // 添加模型信息
    };
  ...
  function addAssistantMessage(content: string, thinking?: string): Message {
    const message: Message = {
      ...
      thinking,
      ...(currentModel.value ? { modelInfo: currentModel.value } : {})
    };
    messages.value.push(message);
    return message;
  }
